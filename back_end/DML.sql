/* 
   File: DML.sql
   Purpose: This file contains the DML statements for the 3D Scans database, including 
            SELECT, INSERT, UPDATE, and DELETE statements for the various tables and relationships in the database.
...
-- =================
-- READ 3DScans list
SELECT 3DScans.scanID AS 'ID', 
       3DScans.scanDate AS 'Scan Date', 
       3DScans.fileName AS 'File Name', 
       Artifacts.artifactID AS 'Artifact ID', 
       Technicians.techEmail AS 'Technician Contact', 
       PointsOfContact.pocEmail AS 'Lab Contact', 
       3DScans.units AS 'Units', 
       3DScans.scanMethod AS 'Scan Method', 
       3DScans.derived 
    FROM 3DScans 
...
SELECT Artifacts.artifactID AS 'ID', 
       ...
       Artifacts.classification AS 'Classification', 
       Artifacts.cultural AS 'Cultural', 
       Artifacts.archaeology AS 'Archaeology' 
    FROM Artifacts 
...
SELECT techID AS 'ID', 
       CONCAT(techFName, ' ', techLName) AS 'Technician Name', 
       techEmail AS 'Email', 
       techPhone AS 'Phone Number' 
    FROM Technicians;
...
SELECT pocID AS 'ID', 
       CONCAT(pocFName, ' ', pocLName) AS 'Contact Name', 
       pocEmail AS 'Email', 
       pocPhone AS 'Phone Number', 
       pocInstitution AS 'Institution', 
       active 
...
UPDATE ScanPOCs SET pocID= :pocIDInput 
